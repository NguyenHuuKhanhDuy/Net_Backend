using System.Text;
using Backend_Net.Application.Common.Interfaces.HttpClient;
using Microsoft.Extensions.Logging;
using Newtonsoft.Json;

namespace Backend_Net.Infrastructure.Services.HttpClientCustom;
...
public class HttpClientCustom<T> : IHttpClientCustom<T> 
{
    ...
    public async Task<TResult?> PostAsync<TResult>(string path, string jsonContent, Dictionary<string, string> headers, CancellationToken cancellationToken)
    {
        var functionName = $"{nameof(IHttpClientCustom<T>)}_{typeof(T).Name}-{path}-{nameof(PostAsync)} =>";
        _logger.LogDebug($"{functionName}");
        ...
        try
        {
            ...
            var result = JsonConvert.DeserializeObject<TResult>(jsonResponse);
            return result;
        }
        catch (Exception ex)
        {
